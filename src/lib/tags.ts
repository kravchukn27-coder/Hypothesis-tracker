// Badge colors for Experiment's 5 tag categories (TECH-003), matching
// the source tool's color scheme.
export const FUNNEL_LEVEL_BADGE_COLOR = "bg-amber-50 text-amber-700 ring-amber-600/20";
export const PLATFORM_BADGE_COLOR = "bg-emerald-50 text-emerald-700 ring-emerald-600/20";
export const CHANNEL_BADGE_COLOR = "bg-blue-50 text-blue-700 ring-blue-600/20";
export const MARKET_BADGE_COLOR = "bg-orange-50 text-orange-700 ring-orange-600/20";
export const PRODUCT_BADGE_COLOR = "bg-purple-50 text-purple-700 ring-purple-600/20";
